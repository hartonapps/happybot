...
`python main.py` is now the normal startup command. If `node_modules/` is missing, it runs `npm install` for the Baileys packages, then starts `whatsapp_adapter.js`.
...
After the first pairing, start the bot with:

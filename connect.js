...
async function connect(attempt = 1, preferredMethod = null) {
  ...
  if (state.creds.registered) {
    console.log('WhatsApp session already exists in auth_info/. No pairing needed.');
    process.exit(0);
  }

  const loginMethod = preferredMethod || await chooseLoginMethod();
  ...
  sock.ev.on('connection.update', async ({ connection, lastDisconnect, qr }) => {
    ...
    if (qr && loginMethod === 'code' && !pairingRequested) {
    ...
    if (connection === 'open') {
      opened = true;
      console.log('WhatsApp session saved successfully in auth_info/. Now run `python main.py` to start the bot.');
      await sleep(1000);
      process.exit(0);
    }

        ...
        if command:
            for plugin, meta, handler in self.command_handlers.get(command, []):
                ...
                if meta["owner_only"] and not ctx.is_owner:
                    ...
                    self.log.info("Command %s from %s in %s handled by %s", command, event.sender_id, event.chat_id, plugin.name)
...
class StdioHappyBot(HappyBot):
    ...
    async def run_stdio(self) -> None:
        self.load_plugins()
        self._start_background_tasks()
        if self.db.get("reactsave_enabled", False):
            await self.adapter_action("set_reactsave", enabled=True)
    ...
    async def adapter_action(self, action: str, **kwargs: Any) -> None:
        self._write_action({"action": action, **kwargs})

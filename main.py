...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run HappyBot core or stdio bridge mode.")
    parser.add_argument("--stdio", action="store_true", help="Read events as JSON lines and write bot actions as JSON lines.")
    args = parser.parse_args()
    logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(name)s: %(message)s")
    bot: HappyBot = StdioHappyBot() if args.stdio else HappyBot()
    if args.stdio:
        asyncio.run(bot.run_stdio())  # type: ignore[attr-defined]
    else:
